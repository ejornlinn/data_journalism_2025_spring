...
**Answer**
There are 8 columns and 526 rows of data. The columns are called OBJECTID <int>, ZIPCODE2 <chr>, ZIPNAME <chr>, EXISTING <chr>, ZIPCODE1 <chr>, Shape_Leng <dbl>, Shape_Area <dbl>, and geometry <MULTIPOLYGON [m]>. 
...
**Answer**
The output contains Maryland zip code data, most likely a boundary table as we are now working with geographical data. ZIPCODE1 seems to be standard 5 digit zip codes and ZIPCODE2 is ZCTA codes which we've worked with before. Shape_Area and Shape_Leng must correspond with the physical data mapping. 
...
**Answer**
Now I see a map of Maryland, most likely divided into counties. There are latitude and longitude markings across the map. The map is gray and white and not in color, pretty basic.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
At first it told me that I needed to restart R to download tigris, but then it work. Not too much happened. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
There are 3,235 rows and 18 columns called STATEFP, COUNTYFP, COUNTYNS, GEOID, GEOIDFQ, NAME, NAMELSAD, LSAD, CLASSFP, MTFCC, CSAFP, CBSAFP, METDIVFP, FUNCTSAT, INTPTLAT AND INTPTLON which are all <chr>, ALAND and AWATER which are <dbl> and finally geometry which is <MULTIPOLYGON [°]>
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
I got a dataset called md_counties that keeps only rows where the STATEFP code equals 24. Upon further research, that correlates to Maryland so the data will exclude states other than Maryland.  
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
I got a map with what I assume to be Maryland counties, it is much simplier and more easy to understand than the previous map but is layed out very similarly. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
This plots the Maryland county boundaries as the background layer and then the Maryland zip code boundaries on top. The theme is still very simple like the other ones. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
Now I see a dataset called foreclosure_zip which has the zip codes that correlate to foreclosure notices recorded in March 2023. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
Now the md_zips data is merged with the foreclosure dataset and it was done through the ZIPCODE1 and zip columns in each dataset. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
Now I see a map of the zip code and county boundaries in Maryland that contains the foreclosure rates for March 2023. They are marked by shades of blue with lighter being more foreclosures and darker being less. 
...
**Answer**
It didn't do anything but it opened the libraries and read my key. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
It got 63 rows with 27 columns and this message: Getting data from the 2017-2021 5-year ACS using FIPS code '24' for state 'MD'. I got a dataset called md_county_notices and md_county_population demonstrating the county foreclosure notices and population data for each county. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
I got a dataset containing the population estimates for every county in Maryland. Each row represents on county and includes the counties name, GEOID, population estimate, more, and geographic boundary. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
I see a date column that has the date in MM/DD/YYYY format along with the time. It also has type which is Notice of Intent to Foreclose. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
This code takes the original ACS population data and cleans it so that the final object only contrains county, population and geometry which is the county's spatial boundary. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
Now my Maryland county population data is combined with the foreclosure notice data by the county columns. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
Now there is a per-capital foreclosure rate included in the data, sorted by highest to lowest rate. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
This code created a map of Maryland counties where each county is colored according to its foreclosure rate per 1,000 residents. County names are labled at the center of each county but are difficult to read to do the placement and coloring.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**
This changed the colors so that they are black, purple, pink, orange and yellow (increasing). The county names are no longer there. 
...
**Answer**
This plot now longer represents th eraw rate, but makes it so that differences between counties with smaller or moderate rates are more visible. Color differences now show proportional changes instead of exact differences. 
