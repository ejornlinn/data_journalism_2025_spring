...
**Answer**
The code gave me a dataframe called alerts with 15 columns, specifically a column called text in which I am assuming is where the original data was extracted from? It also has a filename which must be the file where the text came from. There is also a url column which might be a link to the file or the text?
...
**Answer**
I think the unique function removes multiple repeats of a value so that you only see each item and keeps the first occurence. 
...
**Answer**
unnest_tokens seems to have taken the text column and broken it into individual words while also changing everything to lowercase and removing punctuation.
...
**Answer**
The resulting graphic is a "count of unique words found in Maryland doctor discipline alerts" bar chart. It organizes the unique words from highest to lowest, highest being "the" and lowest being "her." I personally don't think this data is very interesting, it is in black and white and not very engaging. It also is literally counting the words found in Maryland doctor discipline alerts which I don't think is a very engaging topic. 
...
**Answer**
This code is definitely more interesting than the last one as it filtered out the filler words and only includes words that have meaning. I'm assuming than n is the number of times that the word appears and percent is the percentage amount of how many times the word appears in the text. 
...
**Answer**
This code gave us two datasets, unique_words_2023 and unique_words_2022, which list the top unique words from 2023 and the top unique words from 2022. The top five words from 2022 were maryland, board, of, physicians and department. The top five words from 2023 were trevia, hayden, md, harbhajan and ajrawat. I think there may be a few mistakes or something I need to look at further as those words are a bit strange.
...
**Task** Run the following code and describe the results. 
**Answer**
I got back data that gives me the top 10 two-word phrases for 2021. The data has three columns: bigram, n and percent. The phrases are health occ, md code, code ann, summary suspension, medical records, health care, unprofessional conduct, ann health, participant rehabilitation and repl vol. 
...
**Task** Run the following code and describe the results. 
**Answer**
The phrases "summary suspension," "unprofessional conduct" and "medical records" are still among the listed words. "Summary suspension" is still fourth, "unprofessional conduct" is still seventh and "medical records" is still fifth. The datasets are the exact same as far as I can see. 
...
**Task** Run the following code and describe the results. 
**Answer**
Now, a few additional words are included: "individual 4," "prehearing conference," "individual 10" and "respondent's license." "Medical records" is no longer on the list, "summary suspension" is now second and "unprofessional conduct" is now eighth. 
...
**Answer**
Examples like "patient" are counter-intuitive as patient is simply a term used to describe someone needing medical care which can be negative or positive depending on the context. Also there are words like "like" included which don't seem to even be able to be classified as negative or positive. 
