...
**Answer**
umd_courses |> is taking the umd_courses dataset and connecting it to the next function
group_by(term) |> is sorting the rows in the data set by the same term 
summarise(count_classes =n()
) counts how many classes were offered in that term
...
we added arrange(desc(count_classes)) which sorts the data by size of the classes, the terms with the most classes appear at the top instead of chronological order
...
**Answer**
now we are shown the number of classes in each department for each term, sorted by term and then alphabetically by department
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer**
now the code also sorts by the number of total seats offered in each term and sorts the table from largest to smallest
...
we can now see the total, mean and median seats for each term and it sorts by total seats instead of number of classes.
the top terms all have the most seats, meaning they had larger or more courses. 
...
**Answer**
the data is now grouped by department instead of term. the results are sorted by total seats in descending order, departments with the most seats appear at the top.
...
1.  Which departments have offered the largest number of seats across terms consistently?

2.  Do certain departments have more variability in class sizes than others?

3. Which departments have very high enrollment totals and class sizes and may need more resources?
